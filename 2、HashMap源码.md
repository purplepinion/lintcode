...
HashMap继承AbstractMap<K,V>类。

并且实现Map<K,V>, Cloneable, Serializable接口。支持Map操作，克隆，序列化。
...
> 
>
>  HashMap 底层是基于散列算法实现，散列算法分为散列再探测和拉链式。HashMap 则使用了拉链式的散列算法，并在 JDK 1.8 中引入了红黑树优化过长的链表。底层数据结构由`数组+链表`变为了`数组+链表+红黑树`。
>
> 
>
...
public class HashMap<K,V> extends AbstractMap<K,V>
	...
	//阈值8、当桶(bucket)上的结点数大于这个值时会转成红黑树
...
//初始化threshold、找到大于或等于 cap 的最小2的幂。
